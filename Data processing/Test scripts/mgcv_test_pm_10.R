...
# Load the data
data <- read_csv("/Users/sebastianherbst/Dissertation/Data processing/Output/Output - all_air_location_tree/2024-06-28_CM_20m_all_air_tree_data.csv")



#Prepare data
# Convert 'taxon_name' and 'age' to factors if they are not already
data$taxon_name <- as.factor(data$taxon_name)
data$age <- as.factor(data$age)
data$Dist_to_closest_tree <- as.factor(data$Dist_to_closest_tree)
data$Distance_to_tree <- as.factor(data$Distance_to_tree)

# Check for missing values in columns of interest
sapply(data[c("Distance_to_tree", "Dist_to_closest_tree", "taxon_name", "age", "pm_10")], function(x) sum(is.na(x)))

# Filter data to include only rows with no missing values in relevant columns
complete_data <- data %>%
  filter(!is.na(Distance_to_tree) & !is.na(taxon_name) & !is.na(age) & !is.na(Dist_to_closest_tree))



#Create plots of data
# Scatter plot of air quality vs distance to the nearest tree
plot_air_dist <- ggplot(complete_data, aes(x=Distance_to_tree, y=pm_10)) +
  geom_point() +
  theme_minimal() +
  labs(title="Air Quality v Distance to Tree within d",
       x="Distance to  Tree within d",
...
plot_air_dist2 <- ggplot(complete_data, aes(x=Dist_to_closest_tree, y=pm_10)) +
  geom_point() +
  theme_minimal() +
  labs(title="Air Quality v Distance to closest Tree",
       x="Distance to Nearest Tree (meters)",
       y="Air Quality Measure")
ggsave("/Users/sebastianherbst/Dissertation/Data processing/Test scripts/Test output/air_quality_v_dist.png", plot = plot_air_dist2, width = 8, height = 6, dpi = 300)

# Boxplot of air quality by tree name
plot_air_species <- ggplot(complete_data, aes(x=taxon_name, y=pm_10)) +
  geom_boxplot() +
  theme_minimal() +
...
gam_model_pm_10 <- gam(pm_10 ~ s(Distance_to_tree), data=complete_data, method='REML', select=TRUE)
...
predicted_values <- predict(gam_model_pm_10, newdata=complete_data)

# Combine actual and predicted values into a new DataFrame
results <- complete_data %>%
  mutate(predicted_pm_10 = predicted_values)

# Align predictions with the original data
# Map the predicted values back to the original data
data$predicted_pm_10 <- NA
data$predicted_pm_10[complete.cases(data[c("Distance_to_tree", "taxon_name", "age")])] <- predicted_values

# Plot Actual vs Predicted values using the combined DataFrame
plot_act_pred <- ggplot(results, aes(x=pm_10, y=predicted_pm_10)) +
  geom_point() +
  geom_abline(slope=1, intercept=0, color="red") +
  theme_minimal() +
  labs(title="Predicted vs. Actual Air Quality",
       x="Actual Air Quality (pm_10)",
       y="Predicted Air Quality (pm_10)")
...
plot_dist_pol <- plot(gam_model_pm_10, residuals = TRUE, pch = 16, cex = 0.6)
png("/Users/sebastianherbst/Dissertation/Data processing/Test scripts/Test output/plot_dist_pol.png", width = 640, height = 370, units = "px", res = 300) #save plot
dev.off()
...
plot(gam_model_pm_10, pages = 1, all.terms = TRUE, shade = TRUE)
...
plot_res_fit <- ggplot(results, aes(x = predicted_pm_10, y = residuals)) +
  geom_point() +
  geom_hline(yintercept = 0, color = "red") +
  theme_minimal() +
  labs(title = "Residuals vs. Fitted Values",
       x = "Predicted Air Quality (pm_10)",
       y = "Residuals")
