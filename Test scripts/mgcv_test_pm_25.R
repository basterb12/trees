...
# Load the data
data <- read_csv("/Users/sebastianherbst/Dissertation/Data processing/Output/Output - all_air_location_tree/2024-06-17-LL_20m_all_air_tree_data.csv")
...
gam_model_pm_25 <- gam(pm_25 ~ s(Distance_to_tree, k=50), data=complete_data, method='REML')

# Summary of the model
summary(gam_model_pm_25)
...
predicted_values <- predict(gam_model_pm_25, newdata=complete_data)
...
plot_dist_pol <- plot(gam_model_pm_25, residuals = TRUE, pch = 16, cex = 0.6)
...
plot(gam_model_pm_25, pages = 1, all.terms = TRUE, shade = TRUE)



##Compute residuals from the GAM model - patterns in residuals can indicate poor fit
residuals <- residuals(gam_model_pm_25)
